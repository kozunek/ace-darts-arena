...
(function () {
  ...
  function sendMsg(msg, callback) {
    ...
    try {
      browserAPI.runtime.sendMessage(msg, (response) => {
        if (browserAPI.runtime.lastError) {
          console.warn("[eDART] sendMessage error:", browserAPI.runtime.lastError.message);
        }
  ...
  function buildPayloadFromMatch(match, fallbackMatchId) {
    ...
    return {
      ...
      score1, score2,
      avg1: readAvg(s1), avg2: readAvg(s2),
  ...
  checkForHistoryPage();
  console.log("[eDART] Content script loaded (v1.5.0)");
})();

...
import { MessageCircle, Send, ArrowLeft, Users, Search, Trash2, Hash } from "lucide-react";
...
const ChatPage = () => {
  ...
  return (
    <div>
      <PageHeader title="Czat" subtitle="Prywatne wiadomości i kanały grupowe" />
      <div className="container mx-auto px-4 py-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="w-full grid grid-cols-2 mb-4">
            <TabsTrigger value="private" className="gap-1.5">
              <MessageCircle className="h-3.5 w-3.5" /> Prywatne
            </TabsTrigger>
            <TabsTrigger value="group" className="gap-1.5">
              <Hash className="h-3.5 w-3.5" /> Grupowe
            </TabsTrigger>
          </TabsList>
                ...
                <ScrollArea className="flex-1">
                  {(showNewChat || (q && filteredNewPlayers.length > 0)) && (
                    <div className="p-2 border-b border-border bg-muted/20">
                      <p className="text-xs font-display uppercase text-muted-foreground mb-2 px-2">{q ? "Wyniki wyszukiwania" : "Nowa rozmowa"}</p>
                      {filteredNewPlayers.map((p) => (
                        <button key={p.user_id} onClick={() => loadMessages(p.user_id!)} className="w-full flex items-center gap-2 p-2 rounded hover:bg-muted/40 transition-colors text-left">
                          <div className="w-8 h-8 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center text-xs font-display font-bold text-primary">{p.avatar}</div>
                  ...
                  {filteredContacts.map((c) => (
                    <button key={c.user_id} onClick={() => loadMessages(c.user_id)} className={`w-full flex items-center gap-3 p-3 border-b border-border/50 hover:bg-muted/30 transition-colors text-left ${activeChat === c.user_id ? "bg-muted/40" : ""}`}>
                      <div className="w-10 h-10 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center text-sm font-display font-bold text-primary shrink-0">{c.avatar}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="font-body font-semibold text-sm text-foreground">{c.name}</span>
                          {c.unread > 0 && <span className="bg-primary text-primary-foreground text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{c.unread}</span>}
                        </div>
                ...
                {activeChat ? (
                  <>
                    <div className="p-3 border-b border-border flex items-center gap-3">
                      ...
                      <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive h-7 px-2" onClick={async () => {
                        if (!user || !activeChat || !confirm("Czy na pewno chcesz usunąć tę rozmowę?")) return;
                        await supabase.from("chat_messages").delete().eq("sender_id", user.id).eq("receiver_id", activeChat);
                        await supabase.from("chat_messages").delete().eq("sender_id", activeChat).eq("receiver_id", user.id);
